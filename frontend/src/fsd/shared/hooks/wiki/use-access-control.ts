...
import { useRecords, useCreateRecords, useUpdateRecords } from "@/fsd/shared/hooks/tables-mw";

const ACCESS_DATASHEET_ID = process.env.NEXT_PUBLIC_WIKI_ACCESS_DATASHEET_ID || "";
...
export function useUpdatePageAccess() {
  ...
  const updateAccess = useCallback(async (
    ...
    const { records } = await fetch(`/api/wiki/access?pageId=${params.pageId}`).then(r => r.json()).catch(() => ({ records: [] }));
    
    const existing = records?.find((r: { fields: Record<string, unknown> }) => r.fields["Page ID"] === params.pageId);
    ...
    if (existing) {
      result = await updateRecords(ACCESS_DATASHEET_ID, {
        records: [{ recordId: existing.recordId, fields: fields as Record<string, string | number | boolean | null | undefined> }],
        fieldKey: "name",
      });
    } else {
    ...
    if (result?.success) {
      ...
      return parseAccessSettings(result.data?.records?.[0] || { recordId: "", fields: {} });
    } else {
