...
import { useRecords, useCreateRecords } from "@/fsd/shared/hooks/tables-mw";

const VERSIONS_DATASHEET_ID = process.env.NEXT_PUBLIC_WIKI_VERSIONS_DATASHEET_ID || "";
...
export function useAutoSaveVersion(pageId: string | null, content: SerializedEditorState | null) {
  const { createVersion } = useCreateVersion();
  
  const saveVersion = useCallback(async (action: string = "edit", description?: string) => {
