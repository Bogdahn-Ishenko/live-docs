...
const COMMENTS_DATASHEET_ID = process.env.NEXT_PUBLIC_WIKI_COMMENTS_DATASHEET_ID || "";
...
export function useUpdateComment() {
  ...
  const updateComment = useCallback(async (
    ...
    if (result?.success && result.data?.records?.[0]) {
      ...
      toast.success(request.resolved ? "Комментарий решен" : "Комментарий обновлен");
...
export function useDeleteComment() {
  ...
  const deleteComment = useCallback(async (commentId: string): Promise<boolean> => {
    ...
    const result = await updateRecords(COMMENTS_DATASHEET_ID, {
      records: [{ recordId: commentId, fields: fields as Record<string, string | number | boolean | null | undefined> }],
      fieldKey: "name",
    });
...
export function useRestoreComment() {
  ...
  const restoreComment = useCallback(async (commentId: string): Promise<boolean> => {
    if (!COMMENTS_DATASHEET_ID) {
      toast.error("Comments datasheet not configured");
      return false;
    }
